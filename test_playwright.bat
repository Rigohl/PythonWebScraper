...
echo [INFO] Instalando navegadores de Playwright (si Playwright está disponible)...
where playwright >nul 2>&1
if %errorlevel% neq 0 (
    echo [WARN] Playwright no encontrado en PATH. Para pruebas rápidas puedes usar: python -m src.main --demo
    pause
    exit /b 0
)
echo [INFO] Ejecutando 'playwright install'...
playwright install
