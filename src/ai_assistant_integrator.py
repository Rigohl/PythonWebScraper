...
class AIAssistantIntegrator:
    ...
    async def initialize_system(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                self.document_generator = DocumentGenerator(self.config.get('documents', {}))
            ...
            # Estado final
            active_components = sum(1 for status in initialization_report['components'].values()
                                  if status == 'success')

            if active_components >= 2:  # Al menos 2 componentes activos
                self.system_status['initialized'] = True
                initialization_report['status'] = 'success'
                logger.info(f"✅ Sistema inicializado: {active_components}/4 componentes activos")
            else:
                initialization_report['status'] = 'partial'
                logger.warning(f"⚠️ Inicialización parcial: {active_components}/4 componentes activos")
    ...
    async def process_user_request(self, request: str, interaction_mode: str = 'text') -> Dict[str, Any]:
        ...
        try:
            logger.info(f"🤖 Procesando solicitud ({interaction_mode}): {request[:100]}...")
            ...
            for action in actions:
                try:
                    if action['type'] == 'search':
                        ...
                        if search_data:
                            result = await self._execute_document_action(action, search_data)
                            session['results']['document'] = result
                            session['components_used'].append('documents')

                    elif action['type'] == 'voice_response':
                        result = await self._execute_voice_action(action, session['results'])
                    ...
                    session['results'][action['type']] = {'status': 'error', 'error': str(e)}
            ...
            logger.info(f"✅ Solicitud procesada usando: {', '.join(session['components_used'])}")
    ...
    async def _analyze_user_intention(self, request: str) -> Dict[str, Any]:
        ...
        if any(word in request_lower for word in ['busca', 'encuentra', 'información', 'datos']):
            intentions['primary_intent'] = 'search'
            intentions['search_required'] = True

        if any(word in request_lower for word in ['documento', 'informe', 'reporte', 'generar', 'crear']):
            intentions['document_generation'] = True

        if any(word in request_lower for word in ['lee', 'dime', 'habla', 'voz', 'audio']):
            intentions['voice_response'] = True

        if any(word in request_lower for word in ['analiza', 'compara', 'evalúa', 'estudia']):
    ...
    def _simple_intention_analysis(self, request: str) -> List[Dict[str, Any]]:
        ...
        if any(word in request_lower for word in ['documento', 'informe', 'reporte', 'generar']):
            actions.append({
    ...
    def _determine_actions(self, intention_analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if intention_analysis.get('search_required', True):
            search_action = {
                ...
                'priority': 1,
                'params': {
                    'max_results': 8 if intention_analysis.get('analysis_type') == 'detailed' else 5,
        ...
        if intention_analysis.get('document_generation', False):
            doc_action = {
                ...
                'priority': 2,
                'params': {
                    'document_type': 'research_report' if intention_analysis.get('analysis_type') == 'detailed' else 'summary',
    ...
    def _extract_keywords(self, text: str) -> List[str]:
        ...
        stop_words = {
            ...
            'sobre', 'como', 'más', 'pero', 'sus', 'me', 'hasta', 'hay', 'donde',
            'busca', 'encuentra', 'dime', 'información', 'datos'
        }
    ...
    async def _execute_search_action(self, action: Dict[str, Any], request: str) -> Dict[str, Any]:
        ...
        try:
            search_result = await self.search_engine.intelligent_search(
                ...
                content_types=search_params.get('content_types', ['definition', 'examples'])
            )
    ...
    async def _execute_document_action(self, action: Dict[str, Any], search_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            generation_result = await self.document_generator.generate_comprehensive_document(
    ...
    async def _execute_voice_action(self, action: Dict[str, Any], results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if content_to_speak:
                voice_response = await self.voice_assistant.speak_content(content_to_speak)
                return voice_response
            else:
                return {'status': 'no_content', 'message': 'No hay contenido para sintetizar'}

        except Exception as e:
            logger.error(f"Error en respuesta de voz: {e}")
            return {'status': 'error', 'error': str(e)}

    async def _execute_analysis_action(self, action: Dict[str, Any], request: str) -> Dict[str, Any]:
            ...
            for file_info in generated_files:
                if file_info.get('status') == 'success':
                    response['artifacts_generated'].append({
                        ...
                        'location': file_info.get('path', file_info.get('url', 'Unknown'))
                    })

        # Recomendaciones basadas en resultados
        if response['sources_consulted'] > 5:
            response['recommendations'].append('Los datos recopilados son suficientes para análisis profundo')

        if 'document' in results:
            response['recommendations'].append('Documentos generados están listos para revisión')
    ...
    async def execute_voice_conversation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if conversation_result.get('user_input'):
                ...
                processing_result = await self.process_user_request(user_request, 'voice')
...
async def quick_ai_request(request: str, config: Dict[str, Any] = None) -> Dict[str, Any]:
...
async def main():
    ...
    if init_result['status'] in ['success', 'partial']:
        ...
        sample_request = input("\n¿Qué información necesitas? (o presiona Enter para ejemplo): ").strip()
