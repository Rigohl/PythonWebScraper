...
class AutoTestingFramework:
    ...
    def _test_response_consistency(self, domain: str, config: Dict[str, Any]) -> TestResult:
        ...
        try:
            ...
            return TestResult(
                ...
                success=consistency,
                duration=time.time() - start_time,
                metrics={'avg_response_time': avg_time, 'variance': max(response_times) - min(response_times)}
            )
        except Exception as e:
            return TestResult(
                test_id=f"response_consistency_{domain}",
    ...
    def _test_anti_bot_detection(self, domain: str, config: Dict[str, Any]) -> TestResult:
        ...
        try:
            ...
            return TestResult(
                ...
                success=score > 0.7,
                duration=time.time() - start_time,
                metrics={'bot_detection_score': score, 'has_realistic_agent': realistic_agent}
            )
        except Exception as e:
            return TestResult(
                test_id=f"anti_bot_detection_{domain}",
    ...
    def get_testing_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'recent_success_rate': sum(1 for t in recent_tests if t.success) / len(recent_tests) if recent_tests else 0,
            'active_simulations': len(self.simulation_cache),
            'baseline_domains': list(self.baseline_metrics.keys()),
            'avg_test_duration': sum(t.duration for t in recent_tests) / len(recent_tests) if recent_tests else 0
        }