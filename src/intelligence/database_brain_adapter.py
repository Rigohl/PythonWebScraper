...
class DatabaseBrainAdapter:
    ...
    def analyze_data_requirements(self, data_description: str, data_sample: Optional[Dict] = None) -> Dict[str, Any]:
        """Analiza los requisitos de datos y recomienda la mejor base de datos."""

        # Analizar características de los datos
        characteristics = self._extract_data_characteristics(data_description, data_sample)
    ...
    def _extract_data_characteristics(self, description: str, sample: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        # Detectar estructura
        if any(word in description_lower for word in ['json', 'document', 'nested', 'flexible']):
            characteristics['structure'] = 'document'
        elif any(word in description_lower for word in ['table', 'relational', 'structured', 'normalized']):
            characteristics['structure'] = 'relational'
        elif any(word in description_lower for word in ['key-value', 'cache', 'session', 'temporary']):
            characteristics['structure'] = 'key_value'
        elif any(word in description_lower for word in ['graph', 'relationship', 'network', 'connected']):
            characteristics['structure'] = 'graph'
        elif any(word in description_lower for word in ['time-series', 'metrics', 'logs', 'events']):
            characteristics['structure'] = 'time_series'

        # Detectar tamaño
        if any(word in description_lower for word in ['big', 'large', 'massive', 'petabyte', 'terabyte']):
            characteristics['size_estimate'] = 'large'
        elif any(word in description_lower for word in ['small', 'tiny', 'minimal', 'kilobyte']):
            characteristics['size_estimate'] = 'small'

        # Detectar patrones de consulta
        if any(word in description_lower for word in ['search', 'text search', 'full-text']):
            characteristics['search_requirements'] = True
            characteristics['query_patterns'].append('full_text_search')

        if any(word in description_lower for word in ['analytics', 'reporting', 'aggregation', 'olap']):
            characteristics['analytics_requirements'] = True
            characteristics['query_patterns'].append('analytics')

        if any(word in description_lower for word in ['real-time', 'live', 'streaming']):
            characteristics['real_time_requirements'] = True

        # Detectar requisitos de consistencia
        if any(word in description_lower for word in ['acid', 'transaction', 'consistency', 'critical']):
    ...
    def _calculate_database_scores(self, characteristics: Dict[str, Any], db_knowledge: Dict[str, Any]) -> Dict[str, float]:
        ...
        for db_type, info in db_knowledge.items():
            ...
            if structure == 'document' and 'document_store' in capabilities:
                score += 0.4
            elif structure == 'relational' and any(cap in capabilities for cap in ['acid_compliance']):
                score += 0.4
            ...
            if characteristics.get('search_requirements') and 'full_text_search' in capabilities:
                score += 0.3

            if characteristics.get('analytics_requirements') and 'analytics' in capabilities:
                score += 0.3

            if characteristics.get('real_time_requirements') and 'real_time' in capabilities:
                score += 0.2

            if characteristics.get('consistency_requirements') == 'strong' and 'acid_compliance' in capabilities:
                score += 0.2
    ...
    def _generate_recommendation(self, scores: Dict[str, float], characteristics: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _explain_recommendation(self, recommendation: Dict[str, Any], characteristics: Dict[str, Any]) -> str:
    ...
    def suggest_database_for_scraping_target(self, target_url: str, expected_data_structure: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'integration_notes': self._generate_integration_notes(recommendation, target_url)
        }
    ...
    def _analyze_domain_type(self, url: str) -> Dict[str, Any]:
        ...
        return {
            'type': detected_type,
            'description': descriptions[detected_type]
        }

    def _generate_integration_notes(self, recommendation: Dict[str, Any], url: str) -> List[str]:
