...
class EnrichmentStore:
    def __init__(
        self, path: str = "data/brain_enrichment.json", max_sessions: int = 5000
    ):
        self.path = Path(path)
        self.path.parent.mkdir(exist_ok=True)
        self.max_sessions = max_sessions
        self.sessions: List[Dict[str, Any]] = []  # Sesiones enriquecidas crudas
        self.domain_index: Dict[str, Dict[str, Any]] = (
            {}
        )  # Estadísticas agregadas por dominio
        ...
        self.hour_success: Dict[int, Dict[str, int]] = (
            {}
        )  # hour -> {'attempts': X, 'success': Y}
        self._loaded = False
        self._load()
    ...
    def add_session(
        ...
        try:
            ...
            structure_hash, tag_counts, complexity_bucket = self._analyze_structure(
                content_html, getattr(result, "content_text", "")
            )
            ...
            session_record = {
                ...
                "response_time": ctx.get("response_time")
                or getattr(result, "response_time", None),
    ...
    def summarize(self) -> Dict[str, Any]:
        ...
        avg_rt = self._avg(
            [s.get("response_time") for s in self.sessions if s.get("response_time")]
        )
        ...
        top_domains = sorted(
            ...
            key=lambda x: (x["success_rate"], x["attempts"]),
            reverse=True,
        )[:5]

        top_patterns = sorted(
            self.pattern_freq.items(), key=lambda x: x[1], reverse=True
        )[:8]
        top_errors = sorted(
            self.error_signatures.items(), key=lambda x: x[1], reverse=True
        )[:5]
        content_mix = sorted(
            self.content_type_stats.items(), key=lambda x: x[1], reverse=True
        )[:6]
        complexity_mix = sorted(
            self.complexity_distribution.items(), key=lambda x: x[1], reverse=True
        )
    ...
    def domain_insight(self, domain: str) -> Dict[str, Any]:
        ...
        for h, hs in data.get("hour_profile", {}).items():
            ...
            hour_profile.append(
                {"hour": h, "attempts": hs["a"], "success_rate": round(rate, 3)}
            )
        hour_profile = sorted(
            hour_profile, key=lambda x: x["success_rate"], reverse=True
        )[:6]
        ...
        return {
            ...
            "patterns": sorted(
                data.get("patterns", {}).items(), key=lambda x: x[1], reverse=True
            )[:10],
            "errors": sorted(
                data.get("errors", {}).items(), key=lambda x: x[1], reverse=True
            )[:5],
    ...
    def _update_domain_index(self, s: Dict[str, Any]):
        ...
        di = self.domain_index.setdefault(
            d,
            {
                "attempts": 0,
                "success": 0,
    ...
    def _update_complexity(self, s: Dict[str, Any]):
        bucket = s.get("complexity")
        if bucket:
            self.complexity_distribution[bucket] = (
                self.complexity_distribution.get(bucket, 0) + 1
            )
    ...
    def _analyze_structure(self, html: Optional[str], text: str):
        ...
        try:
            ...
            structure_hash = hashlib.sha1(signature_str.encode("utf-8")).hexdigest()[
                :12
            ]
