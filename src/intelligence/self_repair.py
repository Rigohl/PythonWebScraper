...
class SelfRepairAdvisor:
    def __init__(self, simple_brain, autonomous_brain, enrichment, overrides: Optional[Dict[str, Any]] = None, knowledge_base=None):
    ...
    def generate(self, limit: int = 25) -> List[Dict[str, Any]]:
        ...
        suggestions = sorted(
            suggestions,
            key=lambda s: (severity_rank.get(s['severity'], 0), round(s.get('confidence', 0), 3)),
            reverse=True
        )
    ...
    def _kb_refs_for_category(self, category: str) -> List[str]:
        mapping = {
            'stability': ['scraping:respect-delays', 'antibot:fingerprint-patterns', 'antibot:retry-strategy'],
    ...
    def _global_metrics(self) -> Dict[str, Any]:
        ...
        if sessions:
            total_rt = sum(s.response_time for s in sessions if s.response_time is not None)
    ...
    def _suggest_domain_backoff(self, gm: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for domain, stats in self.simple_brain.domain_stats.items():
            ...
            if error_rate >= self.thresholds['error_rate_high']:
                ...
                confidence = min(1.0, 0.6 + (error_rate - self.thresholds['error_rate_high']))
                out.append({
                    'id': f'backoff::{domain}',
                    'timestamp': time.time(),
                    'category': 'stability',
                    ...
                    'signals': {'error_rate': error_rate, 'visits': visits, 'errors': errors, 'spike': spike},
    ...
    def _suggest_speed_ups(self, gm: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if global_avg == 0:
            return out
        for domain, intel in getattr(self.autonomous_brain, 'domain_intelligence', {}).items():
            # Dominios altamente estables + rápidos => oportunidad de agresividad
            if intel.success_rate > 0.9 and intel.avg_response_time < global_avg * 0.6 and intel.total_attempts >= 8:
                out.append({
                    'id': f'speedup::{domain}',
                    'timestamp': time.time(),
                    ...
                    'signals': {'domain_success': intel.success_rate, 'domain_avg_rt': intel.avg_response_time, 'global_avg_rt': global_avg},
                    'confidence': 0.72
                })
            # Dominios lentos -> ajuste
            if intel.avg_response_time > global_avg * self.thresholds['slow_response_factor'] and intel.total_attempts >= 5:
                ...
                out.append({
                    ...
                    'signals': {'domain_avg_rt': intel.avg_response_time, 'global_avg_rt': global_avg, 'delta': delta},
                    'confidence': 0.81
                })
        return out
        ...
        for domain, data in self.enrichment.domain_index.items():
            ...
            if drift_score >= self.thresholds['structural_drift_high'] and total >= 5:
                out.append({
                    ...
                    'signals': {'drift_score': drift_score, 'structure_snapshots': total},
        ...
        for domain, data in self.enrichment.domain_index.items():
            ...
            if gain >= self.thresholds['schedule_gain_min'] and best[2] >= 3:
                out.append({
                    ...
                    'signals': {'best_hour': best[0], 'best_rate': best[1], 'avg_rate': avg, 'gain': gain},
                    'confidence': 0.68
                })
        return out
    ...
    def _suggest_extraction_enrichment(self) -> List[Dict[str, Any]]:
        ...
        candidate_fields = [p for p, c in pattern_freq.items() if p.startswith('field_') and c >= 3]
        ...
        for domain, intel in getattr(self.autonomous_brain, 'domain_intelligence', {}).items():
            ...
            if missing and intel.total_attempts >= 5:
                out.append({
                    ...
                    'signals': {'missing_fields': missing[:10], 'domain_attempts': intel.total_attempts},
                    'confidence': 0.63
                })
        return out
    ...
    def _suggest_healing_improvements(self) -> List[Dict[str, Any]]:
        ...
        for domain, data in self.enrichment.domain_index.items():
            ...
            healing_total = sum(c for p, c in pattern_counts.items() if p.startswith('healing_applied_'))
            ...
            if ratio >= self.thresholds['healing_frequency_high']:
                out.append({
                    ...
                    'signals': {'healing_ratio': ratio, 'healing_events': healing_total, 'attempts': attempts},
    ...
    def _suggest_error_root_causes(self) -> List[Dict[str, Any]]:
        ...
        top_errors = sorted(self.simple_brain.error_type_freq.items(), key=lambda x: x[1], reverse=True)[:5]
        for err, count in top_errors:
            ...
            for domain, stats in self.simple_brain.domain_stats.items():
                ...
                for ev in reversed(list(self.simple_brain.recent_events)):
                    if (ev.domain or self.simple_brain._extract_domain(ev.url)) == domain:
                        if dom_total >= 40:
                            break
                        dom_total += 1
            ...
            if affected_domains:
                ...
                out.append({
                    ...
                    'signals': {'error_type': err, 'domains_affected': affected_domains, 'global_count': count},
                    'confidence': round(confidence, 3)
                })
        return out
