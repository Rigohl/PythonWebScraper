...
class MLScrapingIntelligence:
    ...
    def extract_site_features(
        self, url: str, html: str, response_time: float, status_code: int
    ) -> Dict[str, float]:
        ...
        features = {
            ...
            "css_complexity": len(soup.find_all("style"))
    ...
    def _calculate_dom_depth(self, soup) -> int:
        ...
        def depth(element, current=0):
            if not element.children:
                return current
            return max(
                depth(child, current + 1) for child in element.children if child.name
            )
    ...
    def _detect_captcha(self, soup) -> float:
        ...
        classes = " ".join(
            [el.get("class", []) for el in soup.find_all() if el.get("class")]
        )
        ...
        for script in scripts:
            ...
            if len(script_text) > 100:
                ...
                obfuscation_indicators = [
                    len([c for c in script_text if c in "!@#$%^&*()"])
                    / len(script_text),  # Special chars ratio
                    script_text.count("eval")
                    + script_text.count("unescape"),  # Eval usage
                    (
                        len(script_text.split()) / len(script_text)
                        if script_text
                        else 0
                    ),  # Minification
                ]
...
class ReinforcementLearningOptimizer:
    ...
    def get_state_key(self, features: Dict[str, float]) -> str:
        ...
        discrete_features["response_time"] = (
            "fast"
            if features["response_time"] < 1
            else "medium" if features["response_time"] < 3 else "slow"
        )

        discrete_features["complexity"] = (
            "low"
            if features["dom_depth"] < 5
            else "medium" if features["dom_depth"] < 10 else "high"
        )

        discrete_features["anti_bot"] = (
            "low"
            if features.get("has_captcha", 0) < 0.3
            else "medium" if features.get("has_captcha", 0) < 0.7 else "high"
        )
    ...
    def update_q_value(
        self, state_key: str, action: str, reward: float, next_state_key: str = None
    ):
    ...
    def calculate_reward(
        self, success: bool, response_time: float, errors: int
    ) -> float:
...
class PatternClusterer:
    ...
    def __init__(self, eps: float = 0.5, min_samples: int = 2):
        ...
        self.clusterer = (
            DBSCAN(eps=eps, min_samples=min_samples) if SKLEARN_AVAILABLE else None
        )
        self.clusters = {}

    def fit_predict(
        self, sites_features: Dict[str, Dict[str, float]]
    ) -> Dict[str, int]:
        ...
        features_matrix = np.array(
            [list(features.values()) for features in sites_features.values()]
        )
...
class AnomalyDetector:
    ...
    def __init__(self, contamination: float = 0.1):
        ...
        self.detector = (
            IsolationForest(contamination=contamination) if SKLEARN_AVAILABLE else None
        )
...
class AdvancedMLIntelligence:
    ...
    def learn_from_outcome(
        ...
        reward = self.rl_optimizer.calculate_reward(
            success, final_response_time, errors
        )
        self.rl_optimizer.update_q_value(
            recommendation["state_key"], recommendation["rl_recommended_action"], reward
        )
        ...
        logger.info(
            f"ML system learned from {url}: success={success}, reward={reward:.2f}"
        )

    def _calculate_confidence(
        self, features: Dict[str, float], is_anomaly: bool
    ) -> float:
        ...
        anti_bot_score = features.get("has_captcha", 0) + features.get(
            "has_cloudflare", 0
        )
    ...
    def _retrain_models(self):
        ...
        successful_features = [
            sample["features"] for sample in recent_data if sample["outcome"]["success"]
        ]
    ...
    def get_intelligence_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_retrain": (
                max([s.get("timestamp", 0) for s in self.training_data])
