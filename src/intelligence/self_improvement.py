        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                ...
                if module.startswith("."):
                    ...
                    module.startswith(pkg)
                    for pkg in ["requests", "numpy", "pandas", "sklearn"]
                ):
...
class SelfImprovingSystem:
    ...
    def analyze_codebase(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "suggestions_by_priority": self._group_suggestions_by_priority(
                total_suggestions
            ),
    ...
    def auto_improve_step(self) -> Dict[str, Any]:
        ...
        for improvement in improvements_to_apply:
            ...
            if success:
                ...
                if post_tests["all_passed"]:
                    ...
                    logger.info(
                        f"Successfully applied {suggestion['type']} to {file_path}"
                    )
                else:
                    # Revertir cambio si tests fallan
                    logger.warning(
                        f"Tests failed after applying {suggestion['type']}, reverting"
                    )
                    applied_improvements.append(
                        {
                            "file": file_path,
        ...
        improvement_record = {
            ...
            "improvements_successful": len(
                [i for i in applied_improvements if i["success"]]
            ),
    ...
    def _select_improvements(self, analysis: Dict) -> List[Dict]:
        ...
        safe_improvement_types = ["add_docstring", "optimize_imports", "fix_long_lines"]

        for file_path, file_analysis in analysis["detailed_analysis"].items():
            for suggestion in file_analysis.get("suggestions", []):
                if (
                    suggestion["type"] in safe_improvement_types
                    and len(improvements) < self.config["max_changes_per_session"]
                ):
    ...
    def get_improvement_summary(self) -> Dict[str, Any]:
        ...
        total_improvements = sum(
            record["improvements_successful"] for record in self.improvement_history
        )

        recent_improvements = [record for record in self.improvement_history[-10:]]
