...
class LearningSession:
    ...
    def finalize(self):
        ...
        if self.scraped_urls:
            success_rate = sum(1 for url in self.scraped_urls if url["success"]) / len(
                self.scraped_urls
            )
            avg_response_time = sum(
                url["response_time"] for url in self.scraped_urls
        ...
        self.performance_metrics = {
            ...
            "improvements_applied": len(
                [i for i in self.applied_improvements if i["success"]]
            ),
            "improvement_success_rate": (
                len([i for i in self.applied_improvements if i["success"]])
                / len(self.applied_improvements)
                if self.applied_improvements
                else 0
            ),
        }
...
class ContinuousLearningOrchestrator:
    ...
    def _detect_new_patterns(
        self, url: str, ml_recommendation: Dict, success: bool
    ) -> List[Dict]:
        ...
        if (
            features.get("dom_depth", 0) > 15
            and features.get("script_count", 0) > 10
            and success
        ):
            patterns.append(
                {
            ...
            patterns.append(
                {
                    ...
                    "data": {
                        ...
                        "successful_strategy": ml_recommendation[
    ...
    def _analyze_session_trends(self, session: LearningSession):
        ...
        if best_strategy and best_rate > 0.8:
            ...
            logger.info(
                f"Detected high-success strategy trend: {best_strategy} ({best_rate:.2%})"
            )
    ...
    def _trigger_auto_improvement(self):
        ...
        try:
            ...
            if (
                ...
                and improvement_result.get("improvements_applied", 0) > 0
            ):
                for improvement in improvement_result.get("details", []):
                    self.current_session.add_improvement(
                        improvement["type"], improvement["file"], improvement["success"]
                    )

            logger.info(
                f"Auto-improvement completed: {improvement_result.get('status')}"
            )
    ...
    def _background_auto_improvement(self):
        ...
        try:
            if not self.current_session:  # Solo si no hay sesión activa
                improvement_result = self.self_improver.auto_improve_step()
                logger.info(
                    f"Background auto-improvement: {improvement_result.get('status')}"
                )
        except Exception as e:
    ...
    def _analyze_cross_session_patterns(self, sessions: List[Dict]):
        ...
        for strategy, performance_list in strategy_performance.items():
            if len(performance_list) >= 10:  # Suficientes muestras
                success_rate = sum(1 for p in performance_list if p["success"]) / len(
                    performance_list
                )
                avg_response_time = sum(
                    p["response_time"] for p in performance_list
    ...
    def _cleanup_knowledge_base(self):
        ...
        for snippet_id, snippet in self.knowledge_base.snippets.items():
            if (
                snippet.added_ts < old_threshold
                and snippet.quality_score < 0.6
                and snippet.category in ["learned-patterns", "session-trends"]
            ):
        ...
        if snippets_to_remove:
            logger.info(
                f"Cleaned up {len(snippets_to_remove)} old low-quality snippets"
            )
    ...
    def get_learning_summary(self) -> Dict[str, Any]:
        ...
        if self.learning_history:
            total_urls = sum(
                len(session.get("scraped_urls", []))
                for session in self.learning_history
            )
            total_patterns = sum(
                len(session.get("learned_patterns", []))
                for session in self.learning_history
            )
