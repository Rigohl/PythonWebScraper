...
class DocumentGenerator:
    ...
    def setup_google_credentials(self, credentials_path: str = None, 
        ...
        try:
            if service_account_path and os.path.exists(service_account_path):
                ...
                self.google_services = {
                    ...
                    'sheets': build('sheets', 'v4', credentials=self.google_credentials),
                    'slides': build('slides', 'v1', credentials=self.google_credentials),
    ...
    async def generate_comprehensive_document(self, search_data: Dict[str, Any], 
                                            document_type: str = 'research_report',
                                            format_types: List[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for format_type in format_types:
                try:
                    ...
                    elif format_type == 'gdoc' and GOOGLE_API_AVAILABLE and self.google_credentials:
            ...
            successful_files = [f for f in generation_session['generated_files'] 
                              if f['status'] == 'success']
            
            logger.info(f"📄 Documento generado: {len(successful_files)} archivos creados")
    ...
    def _structure_content_for_document(self, search_data: Dict[str, Any], 
    ...
    async def _generate_word_document(self, structured_content: Dict[str, Any], 
                                    topic: str, document_type: str) -> str:
                    ...
                    if results:
                        ...
                        for result in results:
                            if isinstance(result, dict):
                                ...
                                p.add_paragraph(content[:500] + ("..." if len(content) > 500 else ""))
                                if url:
    ...
    async def _generate_excel_document(self, structured_content: Dict[str, Any], 
                                     topic: str, document_type: str) -> str:
    ...
    def _create_excel_data_sheet(self, workbook, organized_content: Dict[str, Any], 
                               sheet_name: str):
        """Crea hoja de datos en Excel"""
        ws = workbook.create_sheet(title=sheet_name[:31])  # Límite de caracteres en Excel
        
        # Headers
        headers = ['Tipo', 'Título', 'Fuente', 'URL', 'Confianza', 'Contenido (Preview)']
        ...
        for content_type, results in organized_content.items():
            for result in results:
                if isinstance(result, dict):
                    ...
                    ws.cell(row=row, column=5, value=f"{result.get('confidence', 0):.1%}")
    ...
    async def _generate_powerpoint_document(self, structured_content: Dict[str, Any], 
                                          topic: str, document_type: str) -> str:
        ...
        metadata = structured_content.get('metadata', {})
        subtitle.text = f"Generado por Web Scraper PRO\n{metadata.get('creation_date', '')}"
    ...
    async def _generate_google_doc(self, structured_content: Dict[str, Any], 
                                 topic: str, document_type: str) -> str:
        ...
        try:
            ...
            document = self.google_services['docs'].documents().create(body=body).execute()
    ...
    async def _generate_markdown_document(self, structured_content: Dict[str, Any], 
                                        topic: str, document_type: str) -> str:
...
class QuickDocumentGenerator:
    ...
    @staticmethod
    def generate_quick_summary(topic: str, content: str, format_type: str = 'md') -> str:
...
async def quick_document_from_search(search_data: Dict[str, Any], 
                                   format_types: List[str] = None) -> Dict[str, Any]:
...
async def main():
    ...
    sample_search_data = {
        ...
        'synthesis': {
            ...
            'recommended_actions': ['create_comprehensive_document', 'generate_presentation']
        },
        'organized_content': {
            'definition': [
                {
    ...
    if result['status'] == 'completed':
        ...
        for file_info in result['generated_files']:
            if file_info['status'] == 'success':
                print(f"📄 {file_info['format'].upper()}: {file_info.get('path', file_info.get('url', 'N/A'))}")
    else:
