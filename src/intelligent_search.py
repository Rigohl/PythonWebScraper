...
class IntelligentSearchEngine:
    ...
    def __init__(self, brain_instance=None):
        ...
        self.search_sources = {
            ...
            "google_search": {"enabled": False, "priority": 2},  # Requiere API key
            "general_web": {
                "enabled": True,
                "priority": 3,
                "sites": [
    ...
    async def intelligent_search(
        self, topic: str, max_results: int = 10, content_types: List[str] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"🔍 Búsqueda completada: {len(search_results)} fuentes, síntesis generada"
            )
    ...
    async def _analyze_topic_with_brain(self, topic: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if "bot" in topic.lower():
                ...
                expanded_terms.extend(
                    ["web scraping", "extracción de datos", "crawling"]
                )
    ...
    async def _organize_content(
        self, search_results: List[Dict[str, Any]], content_types: List[str]
    ...
    async def _synthesize_content(
        self, organized_content: Dict[str, List[Dict[str, Any]]], topic: str
    ) -> Dict[str, Any]:
        ...
        total_sources = sum(
            len(content_list) for content_list in organized_content.values()
        )
        synthesis["sources_count"] = total_sources

        if total_sources == 0:
            synthesis["summary"] = (
                f"No se encontró información suficiente sobre {topic}."
            )
        ...
        for content_type, results in organized_content.items():
            if results and content_type != "other":
                key_points.append(
                    f"Se encontraron {len(results)} fuentes sobre {content_type} de {topic}"
                )
...
La información recopilada incluye definiciones, ejemplos y guías prácticas.
    ...
    async def quick_search(self, topic: str) -> str:
        ...
        search_result = await self.intelligent_search(
            topic, max_results=3, content_types=["definition"]
        )
...
class SearchResultsExporter:
    ...
    def export_search_results(
        ...
        if not output_path:
            ...
            output_path = (
                f"exports/search_{topic.replace(' ', '_')}_{timestamp}.{format_type}"
            )
        ...
        for content_type, results in organized_content.items():
            if results:
                ...
                for i, result in enumerate(results, 1):
                    ...
                    md_content += (
                        f"- **Confianza:** {result.get('confidence', 0):.1%}\n\n"
                    )
...
async def main():
    ...
    if topic.strip():
        ...
        search_result = await engine.intelligent_search(
            topic, max_results=5, content_types=["definition", "examples", "tutorial"]
        )
        ...
        if search_result["status"] == "completed":
            ...
            print(
                f"❌ Error en la búsqueda: {search_result.get('error', 'Error desconocido')}"
            )
