...
import json, os, time, threading, socket, atexit
from pathlib import Path
from urllib.parse import urlparse
from concurrent.futures import ThreadPoolExecutor, as_completed
_DEFAULT_POLICY = {"default":{"concurrency":4,"request_delay_ms":300,"timeout_s":30,"max_retries":3,"respect_robots":False},"per_host":{},"thresholds":{"success_min":0.90,"rate_limit_max":0.05,"server_error_max":0.03}}
_POLICY_PATHS = [Path(".github/scraper-policy.json"), Path("scraper-policy.json")]
def load_policy()->dict:
    p=None
    for path in _POLICY_PATHS:
        if path.exists(): p=json.loads(path.read_text(encoding="utf-8")); break
    if p is None: p=json.loads(json.dumps(_DEFAULT_POLICY))
    ...
    p["per_host"]=p.get("per_host",{}); return p
class RateLimiter:
    def __init__(self,policy:dict): self.policy=policy; self._lock=threading.Lock(); self._last_at={}
    def _delay_ms_for(self,host:str)->int: return int(self.policy.get("per_host",{}).get(host,{}).get("request_delay_ms", self.policy["default"]["request_delay_ms"]))
    def wait(self,host:str):
        dm=self._delay_ms_for(host)
        if dm<=0: return
        with self._lock:
            now=time.perf_counter(); last=self._last_at.get(host,0.0); need=last+(dm/1000.0)-now
            if need>0: time.sleep(need)
            self._last_at[host]=time.perf_counter()
class Retrier:
    def __init__(self,policy:dict): self.max_retries=int(policy["default"]["max_retries"]); self.timeout_s=float(policy["default"]["timeout_s"])
    def backoff(self,attempt:int)->float:
        import random; return min(8.0,0.25*(2**attempt))*(0.75+0.5*random.random())
try:
    import requests; _HAVE_REQUESTS=True
except Exception:
    import urllib.request, urllib.error; _HAVE_REQUESTS=False
def _hostname(u:str)->str:
    try:
        h=urlparse(u).hostname or ""; return h or socket.gethostbyname(u)
...
def fetch(url:str,limiter:RateLimiter,retrier:Retrier,metrics:MetricsLogger|None=None,headers:dict|None=None):
    ...
    for attempt in range(retrier.max_retries+1):
        ...
        try:
            if _HAVE_REQUESTS:
                resp=__import__("requests").get(url, headers=headers, timeout=retrier.timeout_s)
            ...
            return None
def build_runtime(): 
    p=load_policy(); return p, RateLimiter(p), Retrier(p), MetricsLogger()
