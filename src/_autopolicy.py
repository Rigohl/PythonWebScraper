...
class RateLimiter:
    def __init__(self, policy: dict):
        self.policy = policy; self._lock = threading.Lock(); self._last_at = {}
...
class MetricsLogger:
    def __init__(self, base: str | Path | None=None):
        self.logdir = Path(base or ".") / "logs"
        self.logdir.mkdir(parents=True, exist_ok=True)
        self._rows = []; self._lock = threading.Lock()
    def log(self, url, host, status, elapsed_s, attempt, size, error=None):
        row = {"ts": time.time(), "url": url, "host": host, "status": int(status or 0),
               "elapsed_s": float(elapsed_s), "attempt": int(attempt), "size": int(size),
               "ok": bool(200 <= (status or 0) < 300 and not error), "error": error or ""}
    ...
    def summary(self) -> dict:
        ...
        if not rows:
            return {"n_samples":0,"success_rate":1.0,"rate_limit_rate":0.0,"server_error_rate":0.0,"latency_p50_s":0.0,"latency_p95_s":0.0}
        n=len(rows); sts=[r["status"] for r in rows]; lats=sorted([float(r["elapsed_s"]) for r in rows])
        succ = sum(1 for r in rows if r["ok"])/n
        rate429 = sts.count(429)/n
        rate5xx = sum(1 for s in sts if s>=500)/n
        def pct(p):
            if not lats: return 0.0
            k = max(0, min(len(lats)-1, int(p*len(lats))-1))
            return float(lats[k])
        return {"timestamp": int(time.time()), "n_samples": n, "success_rate": round(succ,3),
                "rate_limit_rate": round(rate429,3), "server_error_rate": round(rate5xx,3),
                "latency_p50_s": round(pct(0.50),3), "latency_p95_s": round(pct(0.95),3)}
...
def fetch(url: str, limiter: RateLimiter, retrier: Retrier, metrics: MetricsLogger | None=None, headers: dict | None=None):
    host = _hostname(url)
    headers = headers or {"User-Agent": "PythonWebScraper/auto-policy"}
    for attempt in range(retrier.max_retries + 1):
        t0 = time.perf_counter()
        limiter.wait(host)
        status, content_len, err = None, 0, None
        try:
            if _HAVE_REQUESTS:
                resp = requests.get(url, headers=headers, timeout=retrier.timeout_s)
                status = resp.status_code; content_len = len(resp.content or b"")
                if 200 <= status < 300:
                    dt = time.perf_counter() - t0
                    if metrics: metrics.log(url, host, status, dt, attempt, content_len)
                    return resp
                raise RuntimeError(f"bad status {status}")
            else:
                req = urllib.request.Request(url, headers=headers)
                with urllib.request.urlopen(req, timeout=retrier.timeout_s) as r:
                    status = getattr(r, "status", 200); content = r.read(); content_len = len(content or b"")
                    if 200 <= status < 300:
                        dt = time.perf_counter() - t0
                        if metrics: metrics.log(url, host, status, dt, attempt, content_len)
                        return content
            ...
            err = str(e); dt = time.perf_counter() - t0
