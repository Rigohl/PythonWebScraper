...
def load_policy() -> dict:
    ...
    d["request_delay_ms"] = int(
        os.getenv("SCRAPER_DELAY_MS", d.get("request_delay_ms", 300))
    )
    ...
    d["respect_robots"] = str(
        os.getenv("SCRAPER_RESPECT_ROBOTS", d.get("respect_robots", False))
    ).lower() in ("1", "true", "yes")
...
class RateLimiter:
    ...
    def _delay_ms_for(self, host: str) -> int:
        ...
        return int(
            ph.get("request_delay_ms", self.policy["default"]["request_delay_ms"])
        )
...
class MetricsLogger:
    ...
    def summary(self) -> dict:
        ...
        lats = sorted([float(r["elapsed_s"]) for r in rows])
        ...
        try:
            if _HAVE_REQUESTS:
                ...
                if 200 <= status < 300:
                    ...
                    if metrics:
                        metrics.log(url, host, status, dt, attempt, content_len)
                    return resp
            ...
            dt = time.perf_counter() - t0
            if metrics:
                metrics.log(url, host, status or 0, dt, attempt, content_len, error=err)
            retryable = (status in (429, 500, 502, 503, 504)) or (
                "timed out" in (err or "").lower()
            )
