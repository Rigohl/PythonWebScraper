...
CMDLET_REGEX = re.compile(r'^(get|set|new|remove|start|stop|restart|enable|disable|test|invoke|clear|select|sort|measure|convert|format|write|out|import|export|where|foreach|join|compare)-[A-Za-z][A-Za-z0-9-]+(?:\s.*)?$', re.IGNORECASE)
SAFE_VERBS = re.compile(r'^(get|select|measure|format|where|foreach|join|compare|test|import|export)\b', re.IGNORECASE)
DESTRUCTIVE_VERBS = re.compile(r'^(remove|set|restart|stop|write|out-file|invoke-webrequest|invoke-expression|set-executionpolicy|new-item|remove-item|del|rmdir|rm)\b', re.IGNORECASE)
PIPE_DANGER = re.compile(r'[|]\s*(Invoke-Expression|iex|bash|-c)|[>]{1,2}\s*\S', re.IGNORECASE)

# Edit actions
EDIT_ACTIONS = {
    "replace": ["reemplaza", "replace", "cambia", "modifica", "sustituye", "change", "modify", "update"],
    "append": ["agrega", "añade", "anade", "append", "inserta", "insert", "add", "agregar"],
...
PATH_REGEX = re.compile(r'([A-Za-z]:\\|/)?([\w\-. ]+[/\\])*[\w\-. ]+\.[A-Za-z0-9]+(\.[A-Za-z0-9]+)*')


def extract_terminal_intent(text: str) -> Dict[str, Any]:
    """Extracts terminal intent details from text."""
    result = {"cmdlet": None, "args": None, "is_destructive": False, "danger_tokens": [], "confidence": 0.0, "reason": []}
    text = text.strip()
...
def extract_edit_intent(text: str) -> Dict[str, Any]:
    ...
    result = {"action": "show", "path": None, "from": None, "to": None, "content": None, "confidence": 0.0, "out_of_scope": False, "reason": []}
    text = text.strip()
    # Detect action
    ...
    replace_match = re.search(r'(?:reemplaza|replace|cambia|modifica|sustituye|change|modify|update)\s*["\'`](.+?)["\'`]\s*(?:por|a|con|to|with|by)\s*["\'`](.+?)["\'`]', text, re.IGNORECASE)
    if replace_match:
        result["from"] = replace_match.group(1)
        result["to"] = replace_match.group(2)
        result["confidence"] += 0.3
        result["reason"].append("replace detected")
    append_match = re.search(r'(?:agrega|añade|anade|append|inserta|insert|add|agregar)\s*["\'`](.+?)["\'`]', text, re.IGNORECASE)
    if append_match:
        result["content"] = append_match.group(1)
        result["confidence"] += 0.2
        result["reason"].append("append detected")
    remove_match = re.search(r'(?:elimina|borra|borrar|remove|delete|quitar|sacar)\s*["\'`](.+?)["\'`]', text, re.IGNORECASE)
    if remove_match:
