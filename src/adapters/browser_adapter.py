...
class BrowserAdapter(ABC):
    ...
    @abstractmethod
    async def navigate_to_url(self, url: str, wait_until: str = "domcontentloaded", timeout: int = 30000) -> Optional[Dict[str, Any]]:
    ...
    @abstractmethod
    async def wait_for_load_state(self, state: str = "networkidle", timeout: int = 15000) -> None:
...
class PlaywrightAdapter(BrowserAdapter):
    ...
    def __init__(self, page: Page) -> None:
        if not PLAYWRIGHT_AVAILABLE:
            raise RuntimeError("Playwright no está disponible. Instale con: pip install playwright")
        self.page = page
...
class MockBrowserAdapter(BrowserAdapter):
    ...
    def __init__(self, mock_content: str = "<html><body>Test content</body></html>",
                 mock_url: str = "http://test.com") -> None:
