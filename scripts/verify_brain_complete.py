...
def verify_neural_brain():
    ...
    try:
        ...
        print(f"  ✅ Procesamiento distribuido: {'Funciona' if processing_works else 'Fallo'}")
        
        success = neurons_count > 0 and clusters_count > 0 and total_synapses > 0 and processing_works
...
def verify_reasoning_system():
    ...
    AdvancedReasoningSystem = import_brain_component('advanced_reasoning', 'AdvancedReasoningSystem')
    if not AdvancedReasoningSystem:
        return {'success': False, 'error': 'No se puede importar AdvancedReasoningSystem'}
    ...
    try:
        ...
        premises = ["Si un sitio usa CAPTCHA, entonces detecta bots", "Este sitio usa CAPTCHA"]
        conclusion = "Este sitio detecta bots"
        
        deductive_result = reasoning.deductive_reasoner.apply_modus_ponens(premises, conclusion)
        deductive_works = deductive_result.get('valid', False)
        print(f"  ✅ Razonamiento deductivo: {'Funciona' if deductive_works else 'Fallo'}")
        ...
        print(f"  ✅ Razonamiento inductivo: {'Funciona' if inductive_works else 'Fallo'}")
        
        # Test razonamiento abductivo
        observation = "Scraping bloqueado"
        hypotheses = ["CAPTCHA activado", "IP bloqueada", "Rate limit excedido"]
        
        abductive_result = reasoning.abductive_reasoner.generate_hypotheses(observation, hypotheses)
        abductive_works = len(abductive_result.get('ranked_hypotheses', [])) > 0
        print(f"  ✅ Razonamiento abductivo: {'Funciona' if abductive_works else 'Fallo'}")
        ...
        success = deductive_works and inductive_works and abductive_works and fuzzy_works
...
def verify_memory_system():
    ...
    AdvancedMemorySystem = import_brain_component('advanced_memory', 'AdvancedMemorySystem')
    ...
    try:
        ...
        print(f"  ✅ Memoria episódica: {'Funciona' if episodic_works else 'Fallo'} ({len(memory.episodic_memory.episodes)} episodios)")
        ...
        print(f"  ✅ Memoria semántica: {'Funciona' if semantic_works else 'Fallo'} ({len(memory.semantic_memory.concepts)} conceptos)")
        ...
        print(f"  ✅ Memoria de trabajo: {'Funciona' if working_memory_works else 'Fallo'} (capacidad: {working_memory_capacity})")
        
        # Test consolidación
        consolidation_works = hasattr(memory, 'consolidation_engine') and memory.consolidation_engine is not None
        ...
        success = episodic_works and semantic_works and working_memory_works and consolidation_works
...
def verify_emotional_brain():
    ...
    try:
        ...
        print(f"  ✅ Detección emocional: {'Funciona' if emotion_detected else 'Fallo'} (emoción: {emotion_name})")
        ...
        if emotion_detected:
            ...
            regulation_result = emotional.emotion_regulator.regulate_emotion(emotion, intensity)
            regulation_works = regulation_result.get('strategy_applied', False)
            strategy = regulation_result.get('strategy', 'ninguna')
            print(f"  ✅ Regulación emocional: {'Funciona' if regulation_works else 'Fallo'} (estrategia: {strategy})")
        else:
        ...
        print(f"  ✅ Estados emocionales: {'Funciona' if has_emotional_state else 'Fallo'}")
        
        # Test influencia en memoria
        has_memory_influence = hasattr(emotional, 'memory_influencer')
        print(f"  ✅ Influencia en memoria: {'Funciona' if has_memory_influence else 'Fallo'}")
        
        # Test influencia en decisiones
        has_decision_influence = hasattr(emotional, 'decision_influencer')
        print(f"  ✅ Influencia en decisiones: {'Funciona' if has_decision_influence else 'Fallo'}")
        
        success = emotion_detected and regulation_works and has_emotional_state and has_memory_influence and has_decision_influence
...
def verify_metacognitive_brain():
    ...
    MetacognitiveBrain = import_brain_component('metacognitive_brain', 'MetacognitiveBrain')
    ...
    try:
        ...
        print(f"  ✅ Conocimiento metacognitivo: {'Funciona' if has_knowledge else 'Fallo'}")
        ...
        if has_monitor:
            ...
            print(f"  ✅ Monitoreo metacognitivo: {'Funciona' if monitoring_works else 'Fallo'}")
        else:
        ...
        if has_controller:
            ...
            print(f"  ✅ Control metacognitivo: {'Funciona' if control_works else 'Fallo'} ({len(strategies)} estrategias)")
        else:
            control_works = False
        ...
        print(f"  ✅ Calibración de confianza: {'Funciona' if has_calibration else 'Fallo'}")
        ...
        success = has_knowledge and monitoring_works and control_works and has_reflection and has_calibration
...
def verify_integration():
    ...
    try:
        ...
        print(f"  ✅ Modo de integración: {'Funciona' if has_integration_mode else 'Fallo'} (modo: {integration_mode})")
        ...
        print(f"  ✅ Consciencia: {'Funciona' if has_consciousness and consciousness_enabled else 'Fallo'}")
        ...
        try:
            ...
            print(f"  ✅ Procesamiento de eventos: {'Funciona' if processing_works else 'Fallo'}")
        except Exception as e:
        ...
        success = (has_unified_brain and has_integration_mode and 
                  has_consciousness and consciousness_enabled and 
                  processing_works and state_works)
...
def main():
    ...
    characteristics = {
        "🔗 Conexiones Neurales Reales": results.get("Cerebro Neural (Conexiones Reales)", {}).get('success', False),
        "🧮 Lógica y Razonamiento": results.get("Sistema de Razonamiento (Lógica)", {}).get('success', False),
        "🧠 Memoria y Aprendizaje": results.get("Sistema de Memoria (Aprendizaje)", {}).get('success', False),
        "❤️ Procesamiento Emocional": results.get("Cerebro Emocional (Emociones)", {}).get('success', False),
        "🤔 Consciencia y Self-Awareness": results.get("Cerebro Metacognitivo (Consciencia)", {}).get('success', False),
        "🌐 Conectividad Total": results.get("Integración Híbrida (Conectividad)", {}).get('success', False)
    }
    ...
    print(f"CAPACIDADES ESPECÍFICAS VERIFICADAS: {verified_capabilities}/{total_capabilities}")
    print("-" * 50)
