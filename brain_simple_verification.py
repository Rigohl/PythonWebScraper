...
def verify_reasoning_logic(brain_components):
    ...
    try:
        ...
        deductive_result = reasoning_system.deductive_reasoner.apply_modus_ponens(premises, conclusion)
        print(f"  • Razonamiento deductivo: {'✅' if deductive_result['valid'] else '❌'}")
        ...
        print(f"  • Razonamiento inductivo: {'✅' if len(patterns['patterns']) > 0 else '❌'}")

        # Test razonamiento abductivo
        observation = "El scraping falló"
        hypotheses = ["Anti-bot detection", "Rate limiting", "Server error"]

        hypotheses_result = reasoning_system.abductive_reasoner.generate_hypotheses(observation, hypotheses)
        print(f"  • Razonamiento abductivo: {'✅' if len(hypotheses_result['ranked_hypotheses']) > 0 else '❌'}")
        ...
        success = (deductive_result['valid'] and
                  len(patterns['patterns']) > 0 and
                  len(hypotheses_result['ranked_hypotheses']) > 0 and
                  fuzzy_result['confidence'] > 0)
...
def verify_memory_learning(brain_components):
    ...
    try:
        ...
        print(f"  • Memoria episódica: {'✅' if episodes_count > 0 else '❌'} ({episodes_count} episodios)")
        ...
        print(f"  • Memoria semántica: {'✅' if concepts_count > 0 else '❌'} ({concepts_count} conceptos)")

        # Test memoria de trabajo
        working_memory_capacity = memory_system.working_memory.capacity
        print(f"  • Memoria de trabajo: {'✅' if working_memory_capacity > 0 else '❌'} (capacidad: {working_memory_capacity})")
        ...
        success = (episodes_count > 0 and
                  concepts_count > 0 and
                  working_memory_capacity > 0 and
...
def verify_emotional_processing(brain_components):
    ...
    try:
        ...
        appraisal_result = emotional_brain.appraisal_system.appraise_situation(situation)
        ...
        print(f"  • Influencia en decisiones: {'✅' if decision_influence_active else '❌'}")

        success = (emotion_detected and
                  regulation_applied and
                  memory_influence_active and
                  decision_influence_active)
...
def verify_metacognitive_awareness(brain_components):
    ...
    try:
        ...
        print(f"  • Conocimiento metacognitivo: {'✅' if metacog_knowledge_active else '❌'}")
        ...
        if hasattr(metacognitive_brain, 'controller'):
            ...
            print(f"  • Control metacognitivo: {'✅' if control_active else '❌'} ({len(control_strategies)} estrategias)")
        else:
        ...
        confidence_calibration_active = hasattr(metacognitive_brain, 'confidence_calibrator')
        print(f"  • Calibración de confianza: {'✅' if confidence_calibration_active else '❌'}")

        success = (metacog_knowledge_active and
                  monitoring_active and
                  control_active and
                  reflection_active and
                  confidence_calibration_active)
...
def main():
    ...
    characteristics = {
        'Conexiones Neurales Reales': results.get('Conexiones Neurales', {}).get('success', False),
        'Lógica y Razonamiento': results.get('Lógica y Razonamiento', {}).get('success', False),
        'Memoria y Aprendizaje': results.get('Memoria y Aprendizaje', {}).get('success', False),
        'Procesamiento Emocional': results.get('Procesamiento Emocional', {}).get('success', False),
        'Awareness Metacognitivo': results.get('Awareness Metacognitivo', {}).get('success', False)
    }
