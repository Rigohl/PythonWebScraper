...
def main():
    ...
    if not test_files:
        logger.warning("No se encontraron archivos de pruebas. Pytest podría fallar con 'no tests collected'.")
    ...
    activate_script = os.path.join(venv_path, 'Scripts', 'activate.bat')  # Específico para Windows
    ...
    print("- Si pytest muestra errores de importación, asegúrate de que src/ esté en PYTHONPATH o agrégalo a sys.path en las pruebas.")
    print("- Para timeouts en el orquestador, aumenta el timeout del navegador en settings (ej. 30s).")
    print("- Si Playwright falla, asegúrate de que los navegadores estén instalados; ejecuta 'playwright install' manualmente.")
    print("- Para muchos fallos, crea issues: ej. 'Issue: Mock httpx en test_scraper.py - Pasos: 1. Ejecuta pytest en test_scraper.py, 2. Observa NetworkError, 3. Agrega @patch para httpx.AsyncClient'.")
