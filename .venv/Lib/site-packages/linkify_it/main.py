...
class Match:
    ...
    def __repr__(self):
        return "{}.{}({!r})".format(self.__class__.__module__, self.__class__.__name__, self.__dict__)
...
class LinkifyIt:
    ...
    def _validate_mailto(self, text, pos):
        ...
        if not self.re.get("mailto"):
            self.re["mailto"] = "^" + self.re["src_email_name"] + "@" + self.re["src_host_strict"]
    ...
    def __init__(self, schemas=None, options=None):
        ...
        self.tlds_default = "biz|com|edu|gov|net|org|pro|web|xxx|aero|asia|coop|info|museum|name|shop|рф".split(
            "|"
        )  # noqa: E501
    ...
    def _compile(self):
        ...
        for alias in aliases:
            ...
            self._compiled[alias]["validate"] = self._compiled[self._schemas[alias]]["validate"]
            self._compiled[alias]["normalize"] = self._compiled[self._schemas[alias]]["normalize"]
        ...
        #
        slist = "|".join([_escape_re(name) for name, val in self._compiled.items() if len(name) > 0 and val])

        re_schema_test = "(^|(?!_)(?:[><\uff5c]|" + self.re["src_ZPCc"] + "))(" + slist + ")"
        ...
        self.re["pretest"] = "(" + re_schema_test + ")|(" + self.re["host_fuzzy_test"] + ")|@"
    ...
    def test(self, text):
        ...
        if self._opts.get("fuzzy_link") and self._compiled.get("http:"):
            ...
            matched_tld = re.search(self.re["host_fuzzy_test"], text, flags=re.IGNORECASE)
            if matched_tld:
    ...
    def normalize(self, match):
        ...
        if match.schema == "mailto:" and not re.search("^mailto:", match.url, flags=re.IGNORECASE):
