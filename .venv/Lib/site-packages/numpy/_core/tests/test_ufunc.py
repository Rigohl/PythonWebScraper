...
class TestUfunc:
    ...
    @pytest.mark.parametrize("matrix", [None, np.array([[1.0 + 1j, 0.5, -0.5j], [0.25, 2j, 0.0], [4.0, 0.0, -1j]])])
    ...
    def test_vecdot_matvec_vecmat_complex(self, ufunc, shape1, shape2, conj):
        ...
        expected1 = np.array(((arr1.conj() if conj else arr1) * arr2).sum(), ndmin=min(len(shape1), len(shape2)))
        ...
        expected2 = np.array(((arr2.conj() if conj else arr2) * arr1).sum(), ndmin=min(len(shape1), len(shape2)))
        assert_array_equal(actual2, expected2)

        actual3 = ufunc(arr1.reshape(shape1).astype("object"), arr2.reshape(shape2).astype("object"))
    ...
    def test_output_ellipsis_errors(self):
        ...
        with pytest.raises(TypeError, match=r"must use `\.\.\.` as `out=\.\.\.` and not per-operand/in a tuple"):
            np.negative(1, out=(...,))
    ...
    indexed_ufuncs = [np.add, np.subtract, np.multiply, np.floor_divide, np.maximum, np.minimum, np.fmax, np.fmin]
...
def test_ufunc_input_casterrors(bad_offset):
    value = 123
    arr = np.array([value] * bad_offset + ["string"] + [value] * int(1.5 * ncu.BUFSIZE), dtype=object)
    with pytest.raises(ValueError):
...
@pytest.mark.parametrize(["dt1", "dt2"], [("S", "U"), ("U", "S"), ("S", "d"), ("S", "V"), ("U", "l")])
