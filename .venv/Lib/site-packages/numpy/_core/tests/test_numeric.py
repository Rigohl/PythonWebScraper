...
class TestNonarrayArgs:
    ...
    def test_reshape_shape_arg(self):
        ...
        with pytest.raises(TypeError, match=r"reshape\(\) missing 1 required positional " "argument: 'shape'"):
            np.reshape(arr)
    ...
    def test_std_with_mean_keyword_keepdims_true_masked(self):
        ...
        A = ma.array(
            [[2.0, 3.0, 4.0, 5.0], [1.0, 2.0, 3.0, 4.0]], mask=[[True, False, True, False], [True, False, True, False]]
        )
...
class TestNonzero:
    ...
    @pytest.mark.parametrize(
        "dtype", [bool, np.int8, np.int16, np.int32, np.int64, np.uint8, np.uint16, np.uint32, np.uint64]
    )
...
class TestClip:
    ...
    def test_clip_min_max_args(self):
        ...
        with assert_raises_regex(TypeError, "missing 1 required positional " "argument: 'a_min'"):
            np.clip(arr, a_max=2)
        msg = "Passing `min` or `max` keyword argument when `a_min` and " "`a_max` are provided is forbidden."
...
class TestIsclose:
    ...
    def test_ip_all_isclose(self):
        ...
        assert np.allclose([1, 2, float("nan")], [1, 2, float("nan")], atol=[1, 1, 1], equal_nan=True)
...
class TestCreationFuncs:
    ...
    def check_function(self, func, fill_value=None):
        ...
        for size, ndims, order, dtype in itertools.product(*par):
            ...
            is_void = dtype is np.dtypes.VoidDType or (isinstance(dtype, np.dtype) and dtype.str.startswith("|V"))
