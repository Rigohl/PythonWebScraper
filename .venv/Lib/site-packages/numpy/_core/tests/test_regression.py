...
class TestRegression:
    ...
    def test_sign_for_complex_nan(self):
        ...
        with np.errstate(invalid="ignore"):
            ...
            have = np.sign(C)
            want = np.array([-1 + 0j, -0.6 + 0.8j, 0 + 0j, 0.8 - 0.6j, 1 + 0j, complex(np.nan, np.nan)])
    ...
    def test_void_scalar_constructor(self):
        ...
        test_record_void_scalar = np._core.multiarray.scalar(test_record.dtype, test_record.tobytes())
    ...
    @pytest.mark.skipif(
        sys.version_info >= (3, 12), reason="Python 3.12 has immortal refcounts, this test no longer works."
    )
    ...
    def test_ticket_1539(self):
        dtypes = [
            x for x in np._core.sctypeDict.values() if (issubclass(x, np.number) and not issubclass(x, np.timedelta64))
        ]
    ...
    def test_load_ufunc_pickle(self):
        ...
        test_data = (
            b"\x80\x04\x95(\x00\x00\x00\x00\x00\x00\x00\x8c\x1cnumpy.core._multiarray_umath\x94\x8c\x03add\x94\x93\x94."
        )
    ...
    def test__array_namespace__(self):
        ...
        with pytest.raises(ValueError, match='Version "2025.12" of the Array API Standard ' "is not supported."):
            arr.__array_namespace__(api_version="2025.12")

        with pytest.raises(ValueError, match="Only None and strings are allowed as the Array API version"):
    ...
    def test_logspace_base_does_not_determine_dtype(self):
        ...
        expected = np.array(
            [[1.0, 3.1621094, 10.0, 31.625, 100.0], [100.0, 31.625, 10.0, 3.1621094, 1.0]], dtype=np.float32
        )
