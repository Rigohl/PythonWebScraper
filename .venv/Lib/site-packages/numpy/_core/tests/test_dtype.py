...
class TestPickling:
    ...
    @pytest.mark.parametrize(
        "DType", [type(np.dtype(t)) for t in np.typecodes["All"]] + [type(np.dtype(rational)), np.dtype]
    )
    ...
    @pytest.mark.parametrize("dt", [np.dtype(t) for t in np.typecodes["All"]] + [np.dtype(rational)])
...
class TestFromCTypes:
    ...
    def test_union(self):
        ...
        expected = np.dtype({"names": ["a", "b"], "formats": [np.uint8, np.uint16], "offsets": [0, 0], "itemsize": 2})
