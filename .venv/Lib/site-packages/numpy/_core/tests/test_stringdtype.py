...
def test_findlike_promoters():
    ...
    for dtypes in [("T", "U"), ("U", "T")]:
        for function, answer in [
            ...
            assert answer == function(np.array(l, dtype=dtypes[0]), np.array(r, dtype=dtypes[1]), s, e)
...
def test_strip_promoter():
    ...
    answer = ["Hello", "Hello??"]
    for dtypes in [("T", "U"), ("U", "T")]:
        result = np.strings.strip(np.array(arg, dtype=dtypes[0]), np.array(strip_char, dtype=dtypes[1]))
...
def test_center_promoter():
    ...
    fillchar = "/"
    for dtypes in [("T", "U"), ("U", "T")]:
        answer = np.strings.center(np.array(arg, dtype=dtypes[0]), 9, np.array(fillchar, dtype=dtypes[1]))
...
@pytest.mark.parametrize("input_data, input_dtype", [(DATETIME_INPUT, "M8[s]"), (TIMEDELTA_INPUT, "m8[s]")])
...
def test_assign_medium_strings():
    ...
    N = 9
    src = np.array(
        (["0" * 256] * 3 + ["0" * 255] + ["0" * 256] + ["0" * 255] + ["0" * 256] * 2 + ["0" * 255]), dtype="T"
    )
    dst = np.array((["0" * 255] + ["0" * 256] * 2 + ["0" * 255] + ["0" * 256] + ["0" * 255] + [""] * 5), dtype="T")
...
def call_func(func, args, array, sanitize=True):
    ...
    if args[0] is None:
        if sanitize:
            san_args = tuple(np.array(arg, dtype=array.dtype) if isinstance(arg, str) else arg for arg in args[1:])
        else:
            san_args = args[1:]
...
@pytest.mark.parametrize(
    "function, expected",
    [(np.strings.find, [[2, -1], [1, -1]]), (np.strings.startswith, [[False, False], [True, False]])],
)
@pytest.mark.parametrize(
    "start, stop", [(1, 4), (np.int8(1), np.int8(4)), (np.array([1, 1], dtype="u2"), np.array([4, 4], dtype="u2"))]
)
