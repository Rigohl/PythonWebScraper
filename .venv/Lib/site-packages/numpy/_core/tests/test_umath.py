...
class TestComparisons:
    ...
    @pytest.mark.parametrize("dtype", sctypes["uint"] + sctypes["int"] + sctypes["float"] + [np.bool])
...
class TestFPClass:
    ...
    def test_fp_noncontiguous(self, dtype):
        data = np.array(
            [np.nan, -np.nan, np.inf, -np.inf, -1.0, 1.0, -0.0, 0.0, 2.2251e-308, -2.2251e-308], dtype=dtype
        )
        ...
        for func, prop in avx_ufuncs.items():
            ...
            for size in range(1, 32):
                ...
                if maxulperr == 0:
                    ...
                    assert_array_max_ulp(myfunc(x_f32), y_true128.astype(np.float32), maxulp=maxulperr)
                    assert_array_max_ulp(myfunc(x_f64), y_true128.astype(np.float64), maxulp=maxulperr)
...
def assert_arctan2_ispinf(x, y):
    assert_(
        (np.isinf(ncu.arctan2(x, y)) and ncu.arctan2(x, y) > 0), f"arctan({x}, {y}) is {ncu.arctan2(x, y)}, not +inf"
    )
...
def assert_arctan2_isnzero(x, y):
    assert_(
        (ncu.arctan2(x, y) == 0 and np.signbit(ncu.arctan2(x, y))), f"arctan({x}, {y}) is {ncu.arctan2(x, y)}, not -0"
    )
