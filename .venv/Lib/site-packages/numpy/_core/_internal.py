...
def _commastring(astr):
    ...
    while startindex < len(astr):
        ...
        if repeats == "":
            ...
            if repeats[0] == "(" and repeats[-1] == ")" and repeats[1:-1].strip() != "" and "," not in repeats:
                warnings.warn(
...
def __dtype_from_pep3118(stream, is_subdtype):
    ...
    while stream:
        ...
        if stream.consume("T{"):
            ...
            raise NotImplementedError(f"Unrepresentable PEP 3118 data type {stream.next!r} ({desc})")
        else:
...
def _gcd(a, b):
    ...
    if not (math.isfinite(a) and math.isfinite(b)):
        raise ValueError("Can only find greatest common divisor of " f'finite arguments, found "{a}" and "{b}"')
    while b:
        a, b = b, a % b
    return a
...
def array_function_errmsg_formatter(public_api, types):
    ...
    return f"no implementation found for '{func_name}' on types that implement " f"__array_function__: {list(types)}"
...
def _ufunc_doc_signature_formatter(ufunc):
    ...
    if ufunc.nout == 0:
        ...
        out_args = "[, {positional}], / [, out={default}]".format(
            positional=", ".join(f"out{i + 1}" for i in range(ufunc.nout)), default=repr((None,) * ufunc.nout)
        )
