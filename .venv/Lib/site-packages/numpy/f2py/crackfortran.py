...
def readfortrancode(ffile, dowithline=show, istop=1):
    ...
    while True:
        ...
        if sourcecodeform == "fix":
            ...
            raise ValueError(f"Flag sourcecodeform must be either 'fix' or 'free': {repr(sourcecodeform)}")
        ...
        if "externals" in groupcache[groupcounter]:
            for name in groupcache[groupcounter]["externals"]:
                ...
                if m1:
                    ...
                    if not m:
                        outmess(f"crackline: could not resolve function call for line={repr(line)}.\n")
                        return
    ...
    if case in ["begin", "call", "callfun"]:
        ...
        for e in markoutercomma(ll).split("@,@"):
            try:
                ...
                outmess(f'analyzeline: could not extract name,expr in parameter statement "{e}" of "{ll}"\n')
                continue
        ...
        if m.group("after").strip().lower() == "none":
            ...
            for e in markoutercomma(m.group("after")).split("@,@"):
                ...
                if not m1:
                    outmess(f'analyzeline: could not extract info of implicit statement part "{e}"\n')
                    continue
                m2 = typespattern4implicit.match(m1.group("this"))
                if not m2:
                    outmess(f'analyzeline: could not extract types pattern of implicit statement part "{e}"\n')
                    continue
    ...
    for k in list(block["common"].keys()):
        ...
        for e in block["common"][k]:
            ...
            if m:
                ...
                n = e
                errmess(f'analyzecommon: failed to extract "<name>[(<dims>)]" from "{e}" in common /{k}/.\n')
            comvars.append(n)
...
def buildimplicitrules(block):
    ...
    if "implicit" in block:
        if block["implicit"] is None:
            implicitrules = None
            if verbose > 1:
                outmess(f"buildimplicitrules: no implicit rules for routine {repr(block['name'])}.\n")
        else:
...
def _selected_real_kind_func(p, r=0, radix=0):
    ...
    if machine.startswith(
        ("aarch64", "alpha", "arm64", "loongarch", "mips", "power", "ppc", "riscv", "s390x", "sparc")
    ):
        if p <= 33:
            return 16
    elif p < 19:
...
def get_parameters(vars, global_params={}):
    ...
    for n in param_names:
        if "=" in vars[n]:
            ...
            dimspec = (
                [s.removeprefix("dimension").strip() for s in vars[n]["attrspec"] if s.startswith("dimension")]
                or [None]
            )[0]
...
def param_eval(v, g_params, params, dimspec=None):
    ...
    if len(dimrange) == 1:
        ...
        raise ValueError("param_eval: multidimensional array parameters " f"{dimspec} not supported")
        ...
        if "depend" in vars[a]:
            for d in vars[a]["depend"]:
                if d in vars and "depend" in vars[d] and a in vars[d]["depend"]:
                    errmess(f'vars2fortran: Warning: cross-dependence between variables "{a}" and "{d}"\n')
