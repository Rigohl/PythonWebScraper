...
class Element:
    ...
    def __init__(self, namespace: str | None = None, element: str | None = None) -> None:
        self.namespace = namespace
...
class CombinedSelector:
    ...
    def __repr__(self) -> str:
        ...
        return f"{self.__class__.__name__}[{self.selector!r} {comb} {self.subselector!r}]"
...
def parse_selector(stream: TokenStream) -> tuple[Tree, PseudoElement | None]:
    ...
    while 1:
        ...
        if peek in (("EOF", None), ("DELIM", ",")):
            break
        if pseudo_element:
            raise SelectorSyntaxError(f"Got pseudo-element ::{pseudo_element} not at the end of a selector")
        if peek.is_delim("+", ">", "~"):
            # A combinator
...
def parse_simple_selector(stream: TokenStream, inside_negation: bool = False) -> tuple[Tree, PseudoElement | None]:
    stream.skip_whitespace()
        ...
        if peek.type == "HASH":
            ...
            if stream.peek() != ("DELIM", "("):
                ...
                if repr(result) == "Pseudo[Element[*]:scope]" and not (
                    ...
                    or (len(stream.used) >= 4 and stream.used[-3].type == "S" and stream.used[-4].is_delim(","))
                ):
...
def parse_simple_selector_arguments(stream: TokenStream) -> list[Tree]:
    ...
    while 1:
        ...
        if pseudo_element:
            raise SelectorSyntaxError(f"Got pseudo-element ::{pseudo_element} inside function")
        stream.skip_whitespace()
        next = stream.next()
...
class MatchFunc(Protocol):
    def __call__(self, string: str, pos: int = ..., endpos: int = ...) -> re.Match[str] | None: ...
...
def tokenize(s: str) -> Iterator[Token]:
    ...
    while pos < len_s:
        ...
        if match:
            value = _sub_simple_escape(_replace_simple, _sub_unicode_escape(_replace_unicode, match.group()))
