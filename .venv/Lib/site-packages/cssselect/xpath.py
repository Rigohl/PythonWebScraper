...
class GenericTranslator:
    ...
    def css_to_xpath(self, css: str, prefix: str = "descendant-or-self::") -> str:
        ...
        """
        return " | ".join(
            self.selector_to_xpath(selector, prefix, translate_pseudo_elements=True) for selector in parse(css)
        )
    ...
    def xpath_literal(s: str) -> str:
        ...
        if "'" not in s:
            ...
            parts_quoted = [f'"{part}"' if "'" in part else f"'{part}'" for part in split_at_single_quotes(s) if part]
    ...
    def xpath_pseudo(self, pseudo: Pseudo) -> XPathExpr:
        ...
        method = cast(Optional[Callable[[XPathExpr], XPathExpr]], getattr(self, method_name, None))
        if not method:
    ...
    def xpath_relation_direct_adjacent_combinator(self, left: XPathExpr, right: XPathExpr) -> XPathExpr:
        ...
        return left.add_condition(f"following-sibling::*[(name() = '{right.element}') and (position() = 1)]")
    ...
    def xpath_contains_function(self, xpath: XPathExpr, function: Function) -> XPathExpr:
        ...
        if function.argument_types() not in (["STRING"], ["IDENT"]):
            raise ExpressionError(f"Expected a single string or ident for :contains(), got {function.arguments!r}")
    ...
    def xpath_lang_function(self, xpath: XPathExpr, function: Function) -> XPathExpr:
        if function.argument_types() not in (["STRING"], ["IDENT"]):
            raise ExpressionError(f"Expected a single string or ident for :lang(), got {function.arguments!r}")
        value = cast(str, function.arguments[0].value)
    ...
    def xpath_attrib_includes(self, xpath: XPathExpr, name: str, value: str | None) -> XPathExpr:
        if value and is_non_whitespace(value):
            ...
            xpath.add_condition(f"{name} and contains(concat(' ', normalize-space({name}), ' '), {arg})")
        else:
    ...
    def xpath_attrib_dashmatch(self, xpath: XPathExpr, name: str, value: str | None) -> XPathExpr:
        ...
        # Weird, but true...
        xpath.add_condition(f"{name} and ({name} = {arg} or starts-with({name}, {arg_dash}))")
        return xpath

    def xpath_attrib_prefixmatch(self, xpath: XPathExpr, name: str, value: str | None) -> XPathExpr:
        if value:
            xpath.add_condition(f"{name} and starts-with({name}, {self.xpath_literal(value)})")
        else:
            xpath.add_condition("0")
        return xpath
    ...
    def xpath_attrib_substringmatch(self, xpath: XPathExpr, name: str, value: str | None) -> XPathExpr:
        if value:
            ...
            xpath.add_condition(f"{name} and contains({name}, {self.xpath_literal(value)})")
        else:
            xpath.add_condition("0")
        return xpath
